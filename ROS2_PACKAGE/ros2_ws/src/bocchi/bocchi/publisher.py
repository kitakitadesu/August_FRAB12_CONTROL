...
class KeyStateManager:
    ...
    def __init__(self, debounce_time=0.05, servo_open_angle=0, servo_closed_angle=180):
        self.debounce_time = debounce_time
        self.current_keys = set()
        self.servo_open_angle = max(0, min(180, int(servo_open_angle)))
        self.servo_closed_angle = max(0, min(180, int(servo_closed_angle)))
        self.servo_position = self.servo_open_angle
    ...
    def update_key(self, key_code, is_pressed=True):
        with self.lock:
            ...
            if key_code == 70 and is_pressed:  # F key
                if self.servo_position == self.servo_open_angle:
                    self.servo_position = self.servo_closed_angle
                else:
                    self.servo_position = self.servo_open_angle
...
class MinimalPublisher(Node):
    def __init__(self, webui_mode='both'):
        ...
        self.servo_open_angle = 0
        self.servo_closed_angle = 110
        self.current_servo_angle = self.servo_open_angle

        # Initialize key state manager with servo targets
        self.key_manager = KeyStateManager(
            debounce_time=0.02,
        ...
        # Publish initial servo position so the hardware starts open
        self.publish_servo(self.current_servo_angle)
    ...
    def publish_servo(self, position):
        ...
        try:
            self.current_servo_angle = position
            msg = Int32()
            msg.data = position
